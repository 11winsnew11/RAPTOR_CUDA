...
#include <csignal>
#include <atomic>
#include <random> // Ditambahkan untuk random seed
#include <algorithm> // Ditambahkan untuk std::shuffle (fallback jika perlu)
...
// ... (Fungsi device helper seperti load_found_flag_relaxed, warp_found_ready tetap sama) ...
...
// ... (Kernel tetap persis sama, tidak perlu diubah karena logika perubahan ada di host) ...
...
// Helper: Multiply 256-bit by 64-bit (Host side)
void mul256_u64(const uint64_t a[4], uint64_t b, uint64_t r[4]) {
    unsigned __int128 res;
    uint64_t carry = 0;
    
    // Limb 0
    res = (unsigned __int128)a[0] * b;
    r[0] = (uint64_t)res;
    carry = res >> 64;

    // Limb 1
    res = (unsigned __int128)a[1] * b + carry;
    r[1] = (uint64_t)res;
    carry = res >> 64;

    // Limb 2
    res = (unsigned __int128)a[2] * b + carry;
    r[2] = (uint64_t)res;
    carry = res >> 64;

    // Limb 3
    res = (unsigned __int128)a[3] * b + carry;
    r[3] = (uint64_t)res;
    // Ignore overflow beyond 256 bits as range_len is within 256 bits
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "--vanity-hash160" && i + 1 < argc) vanity_hex = argv[++i];
            ...
            random_mode = true;
            // Cek apakah ada argumen seed setelahnya (opsional)
            if (i + 1 < argc && argv[i+1][0] != '-') {
                random_seed = std::stoull(argv[++i]);
            } else {
                // Generate random seed menggunakan random_device
    ...
    if(random_mode) {
        std::cout << "======= MODE: RANDOM SEARCH (Shuffled Ladder) ========\n";
    ...
    // --- Modifikasi: pick_threads_total harus menghasilkan Power of 2 ---
    auto pick_threads_total_pow2 = [&](uint64_t upper)->uint64_t {
        ...
        // Cari t yang merupakan power of 2
        // Mulai dari power of 2 terbesar <= t
        uint64_t pot = 1ull;
        while (pot <= t) pot <<= 1;
        pot >>= 1; // power of two terbesar yang <= t
    ...
    if (threadsTotal == 0ull) {
        // Fallback jika gagal cari pow2 (seharusnya tidak terjadi jika range besar)
         std::cerr << "Warning: Could not find Power-of-2 thread count, falling back to linear logic.\n";
         threadsTotal = 0; // fallback ke logika lama jika perlu, tapi kita prioritaskan pow2
         // Untuk simplifikasi, kita return error dulu jika ingin strict random mode
         if (random_mode) {
             std::cerr << "Error: Random mode requires threadsTotal to be a power of two.\n";
             return EXIT_FAILURE;
         }
    }
    ...
    // ==========================================================
    if (random_mode) {
        std::cout << "Applying XOR Shuffle mapping...\n";
        // Pre-calculate per_thread_cnt as 256-bit integer is already done (per_thread_cnt)
        
        for (uint64_t i = 0; i < threadsTotal; ++i) {
            // 1. Scramble index
            // Karena threadsTotal adalah power of 2, operasi bitwise aman.
            // Kita mask seed agar sesuai dengan bit-width threadsTotal.
            uint64_t mask = threadsTotal - 1;
            uint64_t scrambled_idx = i ^ (random_seed & mask);
            
            // 2. Hitung offset scalar = scrambled_idx * per_thread_cnt
            uint64_t offset_scalar[4] = {0,0,0,0};
            mul256_u64(per_thread_cnt, scrambled_idx, offset_scalar);
            
            // 3. Start Scalar = range_start + offset
            uint64_t temp_scalar[4];
            add256(range_start, offset_scalar, temp_scalar);
            
            // 4. Sesuaikan dengan half (kernel logic requires +half)
            uint64_t Sc[4];
    ...
    // ... (Sisa code: Copy target, Malloc, Precompute G, Main Loop tetap sama) ...
    ...
    // Kernel: Generate Initial Points (Px, Py) from the (now potentially scrambled) scalars
    {
    ...
    // ... (Info print & Main Loop tetap sama) ...
