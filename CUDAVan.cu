...
// --- Tambahan untuk Vanity Search ---
__constant__ uint32_t c_vanity_len;          // Panjang prefix dalam byte
__constant__ uint32_t c_vanity_prefix_mask;  // Mask untuk 4 byte pertama (Little Endian)
...
)
{
    ...
    // Muat konstanta vanity ke register
...
#pragma unroll
    ...
    // Helper lambda untuk pengecekan vanity (inline manual di device code tidak support, jadi pakai macro/local logic)
    // Kita akan implementasi logikanya langsung di blok pengecekan.
    ...
    while (batches_done < max_batches_per_launch && ge256_u64(rem, (uint64_t)B)) {
        ...
        // --- Blok Pengecekan Vanity ---
        {
            ...
            // 1. Cek 4 byte pertama dengan masking
            // load_u32_le membaca byte 0,1,2,3 ke dalam uint32.
            // Kita mask sesuai panjang vanity (jika < 4 byte).
            uint32_t h_prefix = load_u32_le(h20);
            bool pref = ((h_prefix & vanity_mask) == (target_prefix & vanity_mask));

            if (__any_sync(full_mask, pref)) {
                // 2. Jika 4 byte pertama cocok (dan total len > 4), cek sisa byte
                bool full_match = false;
                if (pref) {
                    if (vanity_len <= 4) {
                        full_match = true;
                    } else {
                        // Cek byte ke-4 sampai ke-(vanity_len-1)
                        full_match = true;
...
int main(int argc, char** argv) {
    ...
    uint8_t target_hash160[20] = {0}; // Zero init
    uint32_t vanity_len = vanity_hex.length() / 2;
    
    // Convert hex to bytes (padding with zeros on the right if needed, though len determines check)
    ...
    // Calculate Mask for the first 4 bytes
    uint32_t vanity_prefix_mask = 0xFFFFFFFFu;
    if (vanity_len < 4) {
        // Create mask. E.g., len=1 (1 byte), mask = 0x000000FF
        // We are comparing Little Endian uint32 loaded from memory.
        // Byte 0 is LSB. Byte 3 is MSB.
        // If len=1, we care about byte 0. Mask = 0x000000FF.
        // If len=2, we care about byte 0,1. Mask = 0x0000FFFF.
        vanity_prefix_mask = (1ULL << (vanity_len * 8)) - 1;
    }

    std::cout << "Searching for Vanity Prefix: " << vanity_hex << " (" << vanity_len << " bytes)" << std::endl;
    // --------------------------------
